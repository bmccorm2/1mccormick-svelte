<script lang="ts">
	...
	location.set(import.meta.env.VITE_ENVIRONMENT + "Life");

	let GRID_SIZE = 30;
	let TICK = 500;
	let grid = new Array(GRID_SIZE).fill(0).map((e) => (e = new Array(GRID_SIZE)));
...
<button on:click={randomGenerator}>Random Generator!</button>
<button on:click={toad}>Toad</button>
<button on:click={blinker}>Blinker</button>
<button on:click={pentaDecathlon}>Penta-Decathlon</button>
...
<div class="center">
	{#each grid as row}
		<div class="row">
			{#each row as cell}
				<div class="square" class:alive={cell === 1} />
			{/each}
		</div>
	{/each}
</div>
