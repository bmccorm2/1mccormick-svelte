<script lang="ts">
	import { Card, CardHeader, CardBody, InputGroup, InputGroupText, Form, Input } from "sveltestrap";
	import { CREATE_CONSUMPTION } from "../queries";
	...
	const createConsumption = (variables: any) => {
	...
	const handleSubmit = () => {
		const variables = {
			...
			gallons: parseFloat(gallons.toString()),
			miles: parseFloat(miles.toString()),
			carId,
			notes: notes || null,
		};

		createConsumption(variables);
	};

	$: {
		if ($result?.data?.createConsumption) {
...
<Card>
	<CardHeader class="text-center fs-4">Input</CardHeader>
	<CardBody>
		<Form>
			<InputGroup>
				<InputGroupText>
			...
			<InputGroup>
				...
				<Input class="form-control" bind:value={notes} placeholder="Notes" />
			</InputGroup>
			<hr />
			<div class="d-flex text-sm">
				<Input placeholder={`MPG: ${mpg}`} class="form-control" readonly />
				<Input placeholder={`PPG: ${ppg}`} class="form-control" readonly />
			</div>
